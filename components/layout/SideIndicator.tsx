...
const SideIndicator = ({ activeChapter, chapters, brandLabel, utcLabel }: SideIndicatorProps) => {
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-12 bg-lt-ink z-40 hidden md:flex flex-col items-center py-12 justify-between overflow-hidden">
          ...
          return (
            <div key={chapter.id} className="relative flex flex-col items-center">
              {isActive ? (
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-lt-ghost/30 hover:bg-lt-ghost transition-colors"></div>
              )}
            </div>
          );
        })}
